...
func TestActivityRingOrderingNewestFirst(t *testing.T) {
	var r activityRing
	for i := 0; i < 3; i++ {
		r.add(ActivityEvent{Path: "f", Bytes: int64(i)})
	}
	got := r.snapshot()
	if len(got) != 3 {
...
func TestActivityRingWraparound(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		r.add(ActivityEvent{Bytes: int64(i)})
	}
	got := r.snapshot()
...
func TestActivityRingConcurrentAppend(t *testing.T) {
	...
	for g := 0; g < 8; g++ {
		...
		go func() {
			defer wg.Done()
			for i := 0; i < 500; i++ {
				r.add(ActivityEvent{Path: "x", Direction: "upload", Bytes: 1})
			}
		}()
	}
	wg.Wait()
...
func TestDaemonResponseOmitemptyForOldStyle(t *testing.T) {
	old := DaemonResponse{OK: true, Online: true, MountPoint: "P:", Uptime: "1s"}
