...
func NewWatcher(syncDir, remotePath string, cacheDB *cache.DB, store *cache.Store,
	evictor *cache.Evictor, v *vfs.VFS, suppress *gosync.Map) (*Watcher, error) {
...
func (w *Watcher) Start() error {
	...
	go w.loop(ctx)
	log.Printf("watcher: monitoring %s", w.syncDir)
	return nil
}
	...
	for {
		...
		case err, ok := <-w.fsWatcher.Errors:
			...
			if errors.Is(err, fsnotify.ErrEventOverflow) {
				log.Printf("watcher: event overflow — rescanning %s", w.syncDir)
				go func() {
...
func (w *Watcher) handleCreate(ctx context.Context, localPath string) {
	...
	if info.IsDir() {
		...
		if err != nil {
			log.Printf("watcher: create dir inode: %v", err)
			return
		}
		w.cacheDB.EnqueueWriteback(id, "mkdir", remotePath, "")
		log.Printf("watcher: new directory %s", remotePath)
...
func (w *Watcher) handleRemove(_ context.Context, localPath string) {
	...
	if existing.SyncStatus != cache.StatusSynced && existing.SyncStatus != cache.StatusConflict {
		...
		log.Printf("watcher: removed local-only %s (was %s)", remotePath, existing.SyncStatus)
		return
	}

	w.cacheDB.EnqueueWriteback(existing.ID, "delete", remotePath, "")
	log.Printf("watcher: queued delete %s", remotePath)
}
...
func (w *Watcher) syncFile(localPath string) {
	...
	if err != nil {
		log.Printf("watcher: read %s: %v", localPath, err)
		return
	}

	hash, err := w.store.Put(data)
	if err != nil {
		log.Printf("watcher: cache put %s: %v", localPath, err)
		return
	}
	if w.evictor != nil {
	...
	if existing, _ := w.cacheDB.GetInodeByPath(remotePath); existing != nil &&
		existing.ContentHash == hash && existing.SyncStatus == cache.StatusSynced {
		return
	}
	...
	id, err := w.cacheDB.UpsertInode(inode)
	if err != nil {
		log.Printf("watcher: upsert inode %s: %v", remotePath, err)
		return
	}

	if rejected {
		log.Printf("watcher: rejected %s: %s", remotePath, reason)
		return
	}

	w.cacheDB.EnqueueWriteback(id, "upload", remotePath, "")
	log.Printf("watcher: queued upload %s (%d bytes)", remotePath, size)
}
