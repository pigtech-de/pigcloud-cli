...
func resolveOwnSigningEdPubInteractive() []byte {
	if cachedSigningPriv == nil {
		GetSigningKeysIfAvailable(func() {})
	}
	return deriveSigningEdPub(cachedSigningPriv)
}
...
func verifySigningPkHistory(blobJSON, ownEdPub []byte) [][]byte {
	var blob signingPkHistoryBlob
	if json.Unmarshal(blobJSON, &blob) != nil || blob.V != 1 || len(blob.Eds) == 0 || blob.SigEd == "" {
		return nil
	}
...
var ownSigningHistoryOnce sync.Once

func seedOwnSigningPkHistory(ownEdPub []byte) {
	ownSigningHistoryOnce.Do(func() {
		resp, err := api.NewClient().FetchEncryptionKeys(context.Background())
		if err != nil || resp == nil || !resp.Success {
			return
		}
		var payload api.E2EEKeysPayload
		if json.Unmarshal(resp.Raw, &payload) != nil || payload.SigningPkHistory == "" {
			return
		}
		...
		for _, ed := range verifySigningPkHistory(blobJSON, ownEdPub) {
