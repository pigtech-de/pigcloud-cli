...
func (r *Reconciler) Reconcile(ctx context.Context) int {
	...
	filepath.WalkDir(r.syncDir, func(path string, entry os.DirEntry, err error) error {
		...
		if r.healEntry(remotePath, entry) {
			healed++
		}
		return nil
	})
	if healed > 0 {
		log.Printf("reconciler: re-queued %d drifted item(s)", healed)
	}
	return healed
}

func (r *Reconciler) healEntry(remotePath string, entry os.DirEntry) bool {
