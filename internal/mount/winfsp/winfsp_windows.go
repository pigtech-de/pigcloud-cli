...
func safeCall(name string, fn func() int) int {
	defer func() {
		if r := recover(); r != nil {
			...
			log.Printf("winfsp PANIC in %s: %v\n%s", name, r, buf[:n])
		}
	}()
	return fn()
}
...
func (fs *pigFS) Open(path string, flags int) (errc int, fh uint64) {
	defer func() {
		if r := recover(); r != nil {
			buf := make([]byte, 4096)
			n := runtime.Stack(buf, false)
			log.Printf("winfsp PANIC in Open(%s): %v\n%s", path, r, buf[:n])
			errc = -cgofuse.EIO
			fh = 0
		}
	}()
	...
	if err := fs.vfs.Open(node); err != nil {
		log.Printf("winfsp: Open(%s) failed: %v", path, err)
...
func (fs *pigFS) Read(path string, buff []byte, ofst int64, fh uint64) int {
	return safeCall("Read", func() int {
		...
		if err != nil {
			log.Printf("winfsp: Read(%s, off=%d, sz=%d) failed: %v", path, ofst, len(buff), err)
			return -cgofuse.EIO
		}
		if data == nil {
			return 0
